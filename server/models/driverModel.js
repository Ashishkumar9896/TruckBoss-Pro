...
async function createDriver(name, phoneNo, address, salary) {
  const [result] = await pool.query(
    ...
    [name, phoneNo, address, salary]
  );
  return result;
}

async function updateDriver(id, name, phoneNo, address, salary, status) {
  const [result] = await pool.query(
    "UPDATE driver_details SET name=?, phone_no=?, address=?, salary=?, status=? WHERE driver_id=?",
    [name, phoneNo, address, salary, status, id]
  );
  return result;
}
